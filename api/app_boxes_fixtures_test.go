...
func setupLiveBoxes(t *testing.T, proc func(cert *rpcs.EncodedBlockCert) error, l *ledger.Ledger) {
	...
	for _, boxName := range boxNames {
		expectedAppBoxes[firstAppid][logic.MakeBoxKey(types.AppIndex(firstAppid), boxName)] = newBoxValue
	...
	for boxName, valPrefix := range appBoxesToSet {
		...
		key := logic.MakeBoxKey(types.AppIndex(firstAppid), boxName)
	...
	for _, boxName := range encodingExamples {
		args := []string{"create", boxName}
		expectedAppBoxes[thirdAppid][logic.MakeBoxKey(types.AppIndex(thirdAppid), boxName)] = newBoxValue
	...
	for _, valPrefix := range encodingExamples {
		...
		key := logic.MakeBoxKey(types.AppIndex(thirdAppid), valPrefix)
