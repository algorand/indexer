...
def getAccountsPage(pageurl, accounts):
    try:
        ...
        raise
    if (response.code != 200) or not response.getheader("Content-Type").startswith(
        "application/json"
    ):
...
def getAccountList(rooturl):
    ...
    dt = time.time() - start
    logger.info(
        "account list: %d accounts in %0.2f seconds, %.1f /s",
        len(accounts),
        dt,
        len(accounts) / dt,
    )
...
def accountRandom(rooturl, accounts, n=1000, minTime=None, maxTime=30):
    ...
    for addr in accounts:
        ...
        if maxTime and (dt > maxTime):
            break
    logger.info(
        "account random: %d accounts in %0.2f seconds, %.1f /s", count, dt, count / dt
    )
...
def accountRecents(rooturl, accounts, n=1000, minTime=None, maxTime=10, ntxns=1000):
    ...
    for addr in accounts:
        ...
        if maxTime and (dt > maxTime):
            break
    logger.info(
        "account recent txns: %d accounts in %0.2f seconds, %.1f /s; %d txns, %.1f txn/s",
        count,
        dt,
        count / dt,
        txcount,
...
def getAssets(rooturl):
    ...
    dt = time.time() - start
    logger.info(
        "asset list: %d assets in %0.2f seconds, %.1f /s",
        len(assets),
        dt,
        len(assets) / dt,
    )
...
def assetTxns(rooturl, assets, n=1000, minTime=None, maxTime=10, ntxns=1000):
    ...
    for assetid in assets:
        atxnsurl[2] = os.path.join(
            rawurl[2], "v2", "assets", str(assetid), "transactions"
        )
        query = {"limit": ntxns}
        atxnsurl[4] = urllib.parse.urlencode(query)
        pageurl = urllib.parse.urlunparse(atxnsurl)
        ...
        if maxTime and (dt > maxTime):
            break
    logger.info(
        "asset txns: %d assets in %0.2f seconds, %.1f /s; %d txns, %.1f txn/s",
        count,
        dt,
        count / dt,
        txcount,
...
def assetBalances(rooturl, assets, n=1000, minTime=None, maxTime=10, ntxns=1000):
    ...
    for assetid in assets:
        ...
        if maxTime and (dt > maxTime):
            break
    logger.info(
        "asset balances: %d assets in %0.2f seconds, %.1f /s; %d bals, %.1f bal/s",
        count,
        dt,
        count / dt,
        balcount,
...
def main():
    ...
    ap.add_argument(
        "--indexer",
        default="http://localhost:8980",
        help="URL to indexer to fetch from, default http://localhost:8980",
    )
