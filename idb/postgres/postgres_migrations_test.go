...
const dbSchema = `-- This file is setup_postgres.sql which gets compiled into go source using a go:generate statement in postgres.go
--
...
func TestConvertBigIntType(t *testing.T) {
	...
	_, err := pdb.Exec(context.Background(), dbSchema)
	require.NoError(t, err)
