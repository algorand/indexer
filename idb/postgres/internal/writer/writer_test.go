...
func txnParticipationQuery(db *pgxpool.Pool, query string) ([]txnParticipationRow, error) {
	...
	for rows.Next() {
		...
		result.addr, err = basics.UnmarshalChecksumAddress(string(buf))
		if err != nil {
			return nil, err
		}
		results = append(results, result)
...
func TestWriterAccountTableBasic(t *testing.T) {
	...
	assert.Equal(t, test.AccountA.String(), string(addr))
...
func TestWriterAssetTableBasic(t *testing.T) {
	...
	assert.Equal(t, test.AccountA.String(), string(creatorAddr))
	{
...
func TestWriterAppTableBasic(t *testing.T) {
	...
	assert.Equal(t, test.AccountA.String(), string(creator))
	{
