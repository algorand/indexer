...
func yieldTransactions(ctx context.Context, block *types.Block, start uint64, payset []types.SignedTxnInBlock, outCh chan []interface{}) error {
	intra := uint(start)
...
// protocol
func AddTransactions(block *types.Block, start uint64, payset []types.SignedTxnInBlock, tx pgx.Tx) error {
	...
	var err0 error
	go func() {
		err0 = yieldTransactions(ctx, block, start, payset, ch)
		close(ch)
	}()
