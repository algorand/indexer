...
func TestEncodeInnerSignedTxnWithAD(t *testing.T) {
	...
	require.Equal(t, expectedRoot, string(EncodeInnerSignedTxnWithAD(stxn)), "We shouldn't call this on a root txn, but if we did... no inner transactions.")
	require.Equal(t, expectedInner1, string(EncodeInnerSignedTxnWithAD(stxn.EvalDelta.InnerTxns[0])))
	require.Equal(t, expectedInner2, string(EncodeInnerSignedTxnWithAD(stxn.EvalDelta.InnerTxns[0].EvalDelta.InnerTxns[0])))
}
