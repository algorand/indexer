...
func insertAccount(t *testing.T, db *pgxpool.Pool, address sdk.Address, trimmedAccountData *models2.Account) {
...
func checkAccount(t *testing.T, db *pgxpool.Pool, address sdk.Address, accountData *models2.Account) {
...
func TestBasic(t *testing.T) {
	for _, i := range []int{1, 2, 3, 4} {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			...
			insertAccount(t, db, makeAddress(1), &models2.Account{
				Participation: models2.AccountParticipation{VoteKeyDilution: 1},
			})
			insertDeletedAccount(t, db, makeAddress(2))
			insertAccount(t, db, makeAddress(3), &models2.Account{
				Participation: models2.AccountParticipation{VoteKeyDilution: 3},
			})
			...
			checkAccount(
				t, db, makeAddress(1),
				&models2.Account{Participation: models2.AccountParticipation{VoteKeyDilution: 1}})
			checkDeletedAccount(t, db, makeAddress(2))
			checkAccount(
				t, db, makeAddress(3),
				&models2.Account{Participation: models2.AccountParticipation{VoteKeyDilution: 3}})
		})
	}
}
...
func TestAccountAssetCount(t *testing.T) {
	...
	insertAccount(t, db, makeAddress(1), &models2.Account{Participation: models2.AccountParticipation{VoteKeyDilution: 1}})
...
func TestAllResourcesMultipleAccounts(t *testing.T) {
	...
	for i := 0; i < numAccounts; i++ {
		insertAccount(t, db, makeAddress(i), &models2.Account{Participation: models2.AccountParticipation{VoteKeyDilution: uint64(i)}})
